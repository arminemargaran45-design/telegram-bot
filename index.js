...
async function initDB() {
  ...
  await pool.query(`ALTER TABLE users_settings ADD COLUMN IF NOT EXISTS digest_time TEXT DEFAULT '09:00'`);
  await pool.query(`ALTER TABLE users_settings ADD COLUMN IF NOT EXISTS digest_sent_date TEXT`);
...
bot.hears('🌐 Язык', async (ctx) => {
  ...
  await ctx.reply('Выбери язык бота:', languageMenu());
});
...
bot.on('text', async (ctx) => {
  ...
  if (state.step === 'language') {
    ...
    if (text.includes('Deutsch')) language = 'de';
    ...
    return ctx.reply('✅ Язык сохранён', menu());
  }
  ...
  if (state.step === 'choose_task') {
    ...
    if (!templates[text]) {
      return ctx.reply('Выбери задачу кнопкой или нажми «✍️ Своя задача».', taskTemplatesMenu());
    }
